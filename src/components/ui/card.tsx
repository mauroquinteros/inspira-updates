...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-4 overflow-hidden rounded-[1.5rem] border border-white/8 bg-card/85 py-5 text-sm text-card-foreground shadow-[0_18px_60px_rgba(3,7,32,0.28)] backdrop-blur-xl has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-4 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-[1.5rem] *:[img:last-child]:rounded-b-[1.5rem]",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-[1.5rem] border-t border-white/8 bg-white/3 p-5 group-data-[size=sm]/card:p-4",
        className
      )}
      {...props}
    />
